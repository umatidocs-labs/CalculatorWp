...
;(function( $, window, document, undefined ){
    ...
    $.loanInfo = function( input ) {
		...
		if(term.match("w") || term.match("d")){
			//
			amount_t = ((Number(amount)*Number(rate))/100)+ Number(amount);
			
			return {
				original_amount: amount,
				payment_amount:amount_t,
				payment_amount_formatted: amount_t,
				num_payments: 1,
				total_payments: amount_t,
				total_payments_formatted: amount_t, 
				total_interest: amount*rate/100,
				total_interest_formatted: amount*rate/100
			}
		}
