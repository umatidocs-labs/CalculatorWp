...
//-----------------------------------gravity to mylender parser -------------------------------------------------------------------------------
	...
	public function display_form_data($param){
		...
		foreach( $calculatorwpDatValue as $keyvalue ){
			...
			$html_data .= '<tr class="calculatorwp_title_feild" ><td><b>'. mvc_model( 'calculatorwpDatKey' )->find_one( array( 'conditions' => array( 'form_id' => $keyvalue->form_id , 'key_id' => $keyvalue->key_id ) ) )->name .'</b></td><td>'. $keyvalue_data_value . '</td></tr>' ;
