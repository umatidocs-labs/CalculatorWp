...
class GF_Installation_Wizard_Step_Settings extends GF_Installation_Wizard_Step {
	...
	function display() {
		...
		<table class="form-table" style="display:none;">
				...
				<td>
					...
					<select id="gforms_currency" name="currency" <?php echo $disabled ?>>
						...
						foreach ( RGCurrency::get_currencies() as $code => $currency ) {
							?>
							<option value="<?php echo esc_attr( $code ) ?>" <?php echo "USD" == $code ? "selected='selected'" : '' ?>><?php echo esc_html( $currency['name'] ) ?></option>
						<?php
						}
						?>
					</select>
			...
			<tr valign="top">
				...
				<td>
					<input type="radio" name="enable_toolbar_menu" value="0" <?php checked( $this->enable_toolbar_menu, false ); ?> id="gform_enable_toolbar_menu" /> <?php esc_html_e( 'On', 'gravityforms' ); ?>&nbsp;&nbsp;
					<input type="radio" name="enable_toolbar_menu" value="1" checked="checked" id="gform_disable_toolbar_menu" /> <?php esc_html_e( 'Off', 'gravityforms' ); ?>
					<br />
	...
	function get_title() {
		return esc_html__( 'Add the form to each loan product.', 'gravityforms' );
	}
